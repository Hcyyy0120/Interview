...
<img src="JUC.assets/JUC-两阶段终止模式.png" style="zoom: 67%;" />
...
* 当 Thread-2 执行 synchronized(obj) 就会将 Monitor 的所有者 Owner 置为 Thread-2，Monitor 中只能有一个 Owner，**obj 对象的 Mark Word 指向 Monitor**，把**对象原有的 MarkWord 存入线程栈中的锁记录**中（轻量级锁部分详解）
  <img src="JUC.assets/JUC-Monitor工作原理1.png" style="zoom:67%;" />
...
* 自旋失败的情况：
  ...
  <img src="JUC.assets/JUC-自旋失败.png" style="zoom:80%;" />
...
![](JUC.assets/JUC-ThreadLocal数据结构JDK8前.png)
...
![](JUC.assets/JUC-ThreadLocal数据结构JDK8后.png)
...
* set()：添加数据，ThreadLocalMap 使用**线性探测法来解决哈希冲突**
  ...
  * 在探测过程中 ThreadLocal 会复用 key 为 null 的脏 Entry 对象，并进行垃圾清理，防止出现内存泄漏
  ...
  ![](JUC.assets/JUC-replaceStaleEntry流程.png)
...
* 探测式清理：沿着开始位置向后探测清理过期数据，沿途中碰到未过期数据则将此数据 rehash 在 table 数组中的定位，重定位后的元素理论上更接近 `i = entry.key & (table.length - 1)`，让**数据的排列更紧凑**，会优化整个散列表查询性能
  ...
  <img src="JUC.assets/JUC-ThreadLocal探测式清理1.png" style="zoom:67%;" />

  <img src="JUC.assets/JUC-ThreadLocal探测式清理2.png" style="zoom:67%;" />
...
* 如果 key 使用强引用：使用完 ThreadLocal ，threadLocal Ref 被回收，但是 threadLocalMap 的 Entry 强引用了 threadLocal，造成 threadLocal 无法被回收，无法完全避免内存泄漏
  ...
  <img src="JUC.assets/JUC-ThreadLocal内存泄漏强引用.png" style="zoom:67%;" />
...
* 如果 key 使用弱引用：使用完 ThreadLocal ，threadLocal Ref 被回收，ThreadLocalMap 只持有 ThreadLocal 的弱引用，所以threadlocal 也可以被回收，此时 Entry 中的 key = null。但没有手动删除这个 Entry 或者 CurrentThread 依然运行，依然存在强引用链，value 不会被回收，而这块 value 永远不会被访问到，也会导致 value 内存泄漏
  ...
  <img src="JUC.assets/JUC-ThreadLocal内存泄漏弱引用.png" style="zoom:67%;" />
...
* 当一个节点入队：
  ...
  ![](JUC.assets/JUC-LinkedBlockingQueue入队流程.png)
...
* `h = head` → `first = h.next` 
  ...
  ![](JUC.assets/JUC-LinkedBlockingQueue出队流程1.png)

* `h.next = h` → `head = first`

  ![](JUC.assets/JUC-LinkedBlockingQueue出队流程2.png)
...
![](JUC.assets/JUC-newSingleThreadExecutor.png)
...
* 请求的共享资源被占用，AQS 用队列实现线程阻塞等待以及被唤醒时锁分配的机制，将暂时获取不到锁的线程加入到队列中
  ...
  <img src="JUC.assets/JUC-AQS原理图.png" style="zoom: 80%;" />
...
<img src="JUC.assets/JUC-ReentrantLock-非公平锁1.png" style="zoom:80%;" />
...
* 接下来进入 addWaiter 逻辑，构造 Node 队列（不是阻塞队列），前置条件是当前线程获取锁失败，说明有线程占用了锁
  ...
  * Node 的创建是懒惰的，其中第一个 Node 称为 **Dummy（哑元）或哨兵**，用来占位，并不关联线程
  ...
  <img src="JUC.assets/JUC-ReentrantLock-非公平锁2.png" style="zoom:80%;" />
...
* 再有多个线程经历竞争失败后：
  ...
  ![](JUC.assets/JUC-ReentrantLock-非公平锁3.png)
...
* 唤醒的线程会从 park 位置开始执行，如果加锁成功（没有竞争），会设置
  ...
  * 原本的 head 因为从链表断开，而可被垃圾回收（图中有错误，原来的头节点的 waitStatus 被改为 0 了）
  ...
  ![](JUC.assets/JUC-ReentrantLock-非公平锁4.png)
...
* 如果这时有其它线程来竞争**（非公平）**，例如这时有 Thread-4 来了并抢占了锁
  ...
  * Thread-1 再次进入 acquireQueued 流程，获取锁失败，重新进入 park 阻塞
  ...
  ![](JUC.assets/JUC-ReentrantLock-非公平锁5.png)
...
* 开始 Thread-0 持有锁，调用 await，线程进入 ConditionObject 等待，直到被唤醒或打断，调用 await 方法的线程都是持锁状态的，所以说逻辑里**不存在并发**
  ...
  ![](JUC.assets/JUC-ReentrantLock-条件变量1.png)
...
* fullyRelease 中会 unpark AQS 队列中的下一个节点竞争锁，假设 Thread-1 竞争成功
  ...
  ![](JUC.assets/JUC-ReentrantLock-条件变量2.png)
...
* 执行 transferForSignal，**先将节点的 waitStatus 改为 0，然后加入 AQS 阻塞队列尾部**，将 Thread-3 的 waitStatus 改为 -1
  ...
  ![](JUC.assets/JUC-ReentrantLock-条件变量3.png)
...
* 补充情况：查询线程 A 查询数据时恰好缓存数据由于时间到期失效，或是第一次查询
  ...
  <img src="JUC.assets/JUC-ReentrantReadWriteLock缓存.png" style="zoom:80%;" />
...
* 获取读锁失败，进入 sync.doAcquireShared(1) 流程开始阻塞，首先也是调用 addWaiter 添加节点，不同之处在于节点被设置为 Node.SHARED 模式而非 Node.EXCLUSIVE 模式，注意此时 t2 仍处于活跃状态
  ...
  <img src="JUC.assets/JUC-ReentrantReadWriteLock加锁1.png" style="zoom: 80%;" />

* 这种状态下，假设又有 t3 r.lock，t4 w.lock，这期间 t1 仍然持有锁，就变成了下面的样子

  ![](JUC.assets/JUC-ReentrantReadWriteLock加锁2.png)
...
* 接下来 t2 调用 setHeadAndPropagate(node, 1)，它原本所在节点被置为头节点；还会检查下一个节点是否是 shared，如果是则调用 doReleaseShared() 将 head 的状态从 -1 改为 0 并唤醒下一个节点，这时 t3 在 doAcquireShared 内 parkAndCheckInterrupt() 处恢复运行，**唤醒连续的所有的共享节点**
  ...
  <img src="JUC.assets/JUC-ReentrantReadWriteLock解锁1.png" style="zoom: 67%;" />
...
* t4 在 acquireQueued 中 parkAndCheckInterrupt 处恢复运行，再次 for (;;) 这次自己是头节点的临节点，并且没有其他节点竞争，tryAcquire(1) 成功，修改头结点，流程结束
  ...
  <img src="JUC.assets/JUC-ReentrantReadWriteLock解锁2.png" style="zoom: 67%;" />
...
<img src="JUC.assets/JUC-CyclicBarrier工作原理.png" style="zoom: 80%;" />
...
* Semaphore 的 permits（state）为 3，这时 5 个线程来获取资源
  ...
  ![](JUC.assets/JUC-Semaphore工作流程1.png)
...
* 这时 Thread-4 释放了 permits，状态如下
  ...
  ![](JUC.assets/JUC-Semaphore工作流程2.png)
...
![ConcurrentHashMap数据结构](JUC.assets/ConcurrentHashMap数据结构.png)
...
* transfer()：数据转移到新表中，完成扩容
  ...
  ![](JUC.assets/JUC-ConcurrentHashMap-LastRun机制.png)
...
* 缺点：Segments 数组默认大小为16，这个容量初始化指定后就不能改变了，并且不是懒惰初始化
  ...
  ![](JUC.assets/JUC-ConcurrentHashMap 1.7底层结构.png)
...
<img src="JUC.assets/JUC-CopyOnWriteArrayList弱一致性.png" style="zoom:80%;" />
...
![](JUC.assets/JUC-ConcurrentSkipListMap数据结构.png)
...
* findPredecessor()：寻找前置节点
  ...
  ![](JUC.assets/JUC-ConcurrentSkipListMap-Put流程.png)
...
* remove()
  ...
  ![](JUC.assets/JUC-ConcurrentSkipListMap-remove流程.png)
...
![](JUC.assets/JUC-ConcurrentLinkedQueue入队操作1.png)

![](JUC.assets/JUC-ConcurrentLinkedQueue入队操作2.png)

![](JUC.assets/JUC-ConcurrentLinkedQueue入队操作3.png)
...
![](JUC.assets/JUC-ConcurrentLinkedQueue出队操作1.png)

![](JUC.assets/JUC-ConcurrentLinkedQueue出队操作2.png)

![](JUC.assets/JUC-ConcurrentLinkedQueue出队操作3.png)
...
<img src="JUC.assets/IO-DMA.png" style="zoom: 50%;" />
...
![](JUC.assets/IO-BIO工作流程.png)

read 调用图示：read、write 都是系统调用指令

<img src="JUC.assets/IO-缓冲区读写.png" style="zoom: 67%;" />
...
![](JUC.assets/IO-sendfile工作流程.png)
...
<img src="JUC.assets/三次握手.png" alt="三次握手" style="zoom: 50%;" />

<img src="JUC.assets/四次挥手.png" alt="四次挥手" style="zoom: 67%;" />
...
* 常用 API：`public Socket accept()`，**阻塞等待**接收一个客户端的 Socket 管道连接请求，连接成功返回一个 Socket 对象
  ...
  <img src="JUC.assets/Netty-TCP三次握手.png" style="zoom:67%;" />
...
![](JUC.assets/BIO工作机制.png)

![](JUC.assets/TCP-工作模型.png)
...
* 位置、限制、容量遵守以下不变式： **0 <= position <= limit <= capacity**
  ...
  <img src="JUC.assets/NIO-Buffer操作.png" style="zoom:67%;" />
...
<img src="JUC.assets/JVM-直接内存直接缓冲区.png" style="zoom: 50%;" />

<img src="JUC.assets/JVM-直接内存非直接缓冲区.png" style="zoom:50%;" />
