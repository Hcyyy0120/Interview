...
public class InheritableThreadLocalDemo {
    public static void main(String[] args) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                System.out.println("子线程获取父父线程threadLocal数据：" + threadLocal.get());
...
+   **加载**：spring会扫描应用程序上下文中的所有类和配置文件，并通过Java的反射机制来加载它们

+   **实例化**：将所有Bean实例化，放到Bean容器，容器负责管理这些Bean的生命周期和依赖关系。

    ​				spring使用反射创建Bean实例，构造 / set / 字段 注入需要的依赖关系

+   **配置**：spring对所有的Bean进行配置，包括属性设置、设置初始化方法......（xml配置、注解配置）

+   **初始化**：调用Bean的初始化方法，可以在xml中通过init-method来指定

+   **后置处理**：对所有Bean进行后置处理，这包括:添加拦截器、代理等。后置处理可以在Bean的定义中通过BeanPostProcessor接口来实现，在Bean初始化前后进行一些额外的处理。

+   **就绪**：在所有的初始化和后置处理都完成之后，Spring就可以将应用程序上下文视为已就绪，可以接受请求了
