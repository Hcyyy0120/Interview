...
+   二级：earlySingletonObjects，**完成实例化，但是还未进行属性注入及初始化的对象**（半成品，生命周期还未走完）
+   三级：singletonFactories，**提前暴露的一个单例工厂，二级缓存中存储的就是从这个工厂中获取到的对象**
...
+   **解决方案一**：**拆分多个 Queue，每个 Queue一个 Consumer**，在应用端进行分片处理，对一组消息的统一标识进行Hash，散列到同一个queue
