...
+   **2、查询缓存**（是service层的query cache，不是InnoDB的buffer pool）
...
+   `m_low_limit_id`（max_trx_id，预分配的事务ID）：**目前出现过的最大的事务 ID+1，即下一个将被分配的事务 ID**。**大于等于**这个 ID 的数据版本均**不可见**
+   `m_up_limit_id`（min_trx_id）：**活跃事务列表 `m_ids` 中最小的事务 ID**，如果 `m_ids` 为空，则 `m_up_limit_id` 为 `m_low_limit_id`。**小于**这个 ID 的数据版本均**可见**
    ...
    +   写操作（写缓存）
        ...
        +   **删除缓存**：于是考虑“先删除缓存，后更新数据库“和”先更新数据库，后删除缓存”，**从操作失败的角度考虑，但凡第二步失败，都会导致数据不一致**。重点看（读写）**并发场景**下
            ...
            +   **由于出错概率极低，所以可以认为是能保证数据一致性的。因此我们应该采用这种方案**。为什么概率极低？
                ...
                +   哪怕真的出错了，造成了一定的不一致，可以采用MQ延迟消息再删一次（双删），具体延迟时间要看业务需求   或者 缓存TTL兜底（毕竟概率真的很小）
...
**TODO 8.1 https://www.bilibili.com/video/BV1jM411A7nW** 3:19
...
**并发标记**是从GC Roots的直接关联对象开始遍历整个对象图的过程，这个过程**耗时较长但是不需要停顿用户线程**，可以与垃圾收集线程一起**并发运行**；
...
+   **无法处理浮动垃圾，有可能出现并发失败（Concurrent Mode Failure）进而导致另一次完全“STW”的Full GC的产生。**
...
这个模型为什么可以实现？这就取决于G1是基于分区回收的（区别于分代回收），分区就是将java的堆空间分为一个个大小相等的Region，G1收集器会跟踪各个Region的价值大小，然后维护一个优先级列表，优先去回收价值收益最大的Region。
...
+   LongAddr：是一个原子累加器，是分段锁的思想，每个段进行CAS的比较
+   AQS：内部有一个state，是一个共享变量，所有想要获取锁的线程，都会尝试CAS的方式去修改state。state在不同的锁，不同的场景下有不同的含义。

##### 原理

Java中的unsafe类，提供了CompareAndSwapInt / Long / Object的native方法 ，这个方法是在硬件层面去实现的

##### 缺陷

+   当竞争激烈的时候，每次CAS都失败，线程是一直处于工作状态，不断进行比较的动作，是忙轮询
+   ABA问题。AtomicStampedReference自带了版本号，可以解决

#### 悲观锁

悲观锁有多种不同的实现，共性是：获取不到锁，线程就去阻塞
...
+   偏向锁
    ...
    +   批量重偏向（20次）：一个锁对象被其他线程访问，会触发撤销偏向，变为轻量级锁。

        +   如：此时有30个对象，分别由线程1加锁，30个对象上此时都是线程1 的偏向锁。

            接下来线程2重新对这些对象循环加锁，执行到第20次时，开始偏向线程2，此后的剩余对象直接偏向线程2

        +   如果一个线程偏向-->撤销-->轻量级20次，那么之后偏向就不会-->轻量级，而是偏向-->重偏向

    +   批量撤销偏向（40次）

        +   当撤销操作达到40次时，此后的对象 / 包括该类创建出来的对象都是不可偏向的

+   轻量级锁

    +   同样锁只会被一个线程持有，只不过是被多个线程在不同的时刻获取到，实际是没有竞争的	
    +   通过CAS 的方式去尝试将栈帧中的锁记录指向他的MarkWord
    +   CAS失败有两种情况
        +   如果是其它线程已经持有了该 Object 的轻量级锁，这时表明有竞争，进入锁膨胀过程
        +   如果是线程自己执行了 synchronized 锁重入，就添加一条 锁记录 作为重入的计数

+   重量级锁

    +   两个优化
        +   自旋锁优化（10次）：一开始是一旦没有获取到锁就被阻塞，放到阻塞队列中。优化后尝试去自旋CAS获取锁，若这10次获取到锁就不需要去阻塞了。(线程阻塞涉及用户态和内核态的切换，是消耗性能的)
...
+   synchronized是java的关键字，是JVM层面的锁，通过锁监视器monitor来实现。

    Lock是java的一个接口，是API层面

+   synchronized 可以给方法 、 代码块解锁

    Lock只能给代码块加锁

+   synchronized不需要手动去获取锁和释放锁，代码块 / 方法执行完毕 / 发生异常自动解锁

    Lock需要手动lock()，unLock()去加锁，释放锁，如果处理不当，就会造成死锁
...
刚开始如果没有synchronized锁没有线程占用那就是无状态锁，然后有个A线程占用了这个锁就会升级为偏向锁，然后如果这个时候A线程又来尝试获取这个锁，那就可以直接获取这个锁，如果是B线程来获取这个锁，那就会从偏向锁升级为轻量级锁， 轻量级锁认为虽然竞争是存在的，但是理想情况下竞争的程度很低，通过自旋方式等待一会儿上一个线程就会释放锁，B线程就会自旋等待A线程释放锁，这个时候如果又来了个C线程，那就会从轻量级锁升级为重量级锁，重量级锁会使除了此时拥有锁的线程以外的线程都阻塞。
...
试图抢占的线程自旋达到阈值，就会停止自旋，那么此时锁就会膨胀成**重量级锁（称为重量级锁的原因就是在锁申请的时候都要有一个在用户态到内核态的转换，monitor是操作系统层面的对象）**。当其膨胀成重量级锁后，其他竞争的线程进来就不会自旋了，而是直接阻塞等待，并且 Mark Word 中的内容会变成一个监视器（monitor）对象，用来统一管理排队的线程。
...
+   其他参数
    ...
    +   `defaultHandler`：**拒绝策略**。线程池阻塞队列已满且线程数超过`maximumPoolSize`之后的拒绝策略。
        ...
        +   DiscardOldestPolicy：丢弃阻塞队列中靠最前的任务，并0执行当前任务
...
简单来说：线程池就是用一堆包装住Thread的Wroker类的set集合，线程（Worker）内部有条件的进行着死循环，从而可以不断接受任务来进行，达到复用的效果。
...
像可见性：其实就是缓存一致性，要保证本地线程内存和主内存中的值保持一致。volatile修饰的变量**在写操作的时候会使用一个Lock指令，去将当前处理器缓存的数据刷新到主内存，同时使得其他处理器缓存的该内存地址的数据无效**。
...
是一个抽象类，就是在Java中去实现锁、同步器的一个核心组件。他内部有一个被volatile修饰的共享变量**state**，state在不同实现中有不同的含义，操作state就是通过CAS的方式。还有一个是**阻塞队列**，基于双向链表实现的，暂时获取不到锁的线程将被加入到该队列中。还有一个**条件队列**，他是个单向链表。相当于Java中wait-notify机制，在ReentrantLock就是await-signal，当获取到锁以后，发现条件不满足，这时调用await，就会去条件队列中等待。还有，AQS自己实现了一个接口**AbstractOwnerableSynchronizer**，AQS自己维护了一个owner，这个owner就是锁的持有者，记录着锁被哪个线程获取到了，是Thread类的一个封装
...
我认为MySQL主要是用于存储，其次才是搜索，而ES是一个专门用来在分布式环境下的搜索与分析引擎。

而且在项目中是使用ES对帖子进行一个搜索，我们平时使用这样的搜索，希望的效果肯定是模糊搜索、进行相关性匹配的，比如莎士比亚，要出现哈姆雷特、罗密欧、英文单词啥的，这样的效果是MySQL的like模糊查询无法实现的。copy_to实现匹配搜索

ES的特点的话，像JSON格式存储数据、基于RESTFul风格进行查询、倒排索引（将存储内容中的单词作为索引，将包含该单词的文档ID作为记录的结构，搜索时进行一个取交集，得到结果）

对title、content这两个字段在Mapping中设置为text，进行模糊查询功能

全文检索查询是先利用分词器对用户输入的内容进行分词，然后去倒排索引库中匹配

分页是由from，size两个参数决定。

ik分词使用 ik_max_word（粒度更细，但占用空间更多，ik_smart粒度更粗，占用空间少），分词底层是使用了词典+自己扩展
...
**一**、MySQL关系型数据库主要还是用于存储，其次才是搜索。而es是一个分布式的搜索与分析引擎。**RESTful**,简洁、易用，URI中没有动词，只有名词（动词由HTTP协议中的GET/POST/PATCH/DELETE来指定），每一个URI代表一种资源
...
在帖子实体类（DiscussPost）上添加注解`@Document(indexName="discusspost",type="_doc",shards=6,replicas=3)`
...
```java
    ...
    //analyzer存储数据时的分词器(分的越细越好,增加搜索范围)
    //searchAnalyzer搜索时的分词器(聪明一点/粒度粗一点)
...
`kafkaTemplate.send(topic, msg)`+自定义`KafkaSendResultHandler`获取详细回调结果，进行重试

或者不进行重试，`kafkaTemplate.send().addCallback()`，这样只能获取到异常信息(Throwable)。
...
public class EventProducer {
    ...
    public void fireEvent(Event event) {
        ...
        ListenableFuture result = kafkaTemplate.send(event.getTopic(), JSONObject.toJSONString(event));
...
使用kafka消息队列（异步发送，可以利用回调函数判断发送是否成功，据此进行失败重试）

canal监听MySQL中binlog日志的变更（也可以增加消息队列重试机制）
...
+   应用场景：
    ...
    +   **消息队列**：项目中就是作为消息队列来使用的，异步、削峰、解耦（搜索帖子和修改帖子  功能上的解耦）
    +   **数据处理：** 构建实时的流数据处理程序来转换或处理数据流

####  kafka的消息模型

是**发布订阅模型**，使用主题(topic)作为消息通信载体，类似于广播模式

发布者发布一条消息，该消息通过主题传递给所有的订阅者，在一条消息广播之后才订阅的用户是收不到该消息的

支持多个消费者

![image-20230415205847484](面试手册随笔.assets/image-20230415205847484.png)
...
+   **发送消息的时候指定 key**（**推荐**，但也仅仅是局部有序，要想全局有序，需要1个Topic只能对应1个Partition，而且对应的consumer也要使用单线程来消费接收或者保证消费顺序的线程模型）

    下图为要想全局有序，但consumer没有使用单线程而造成的消费乱序。

![img](面试手册随笔.assets/7000.png)

#### kafka如何保证消息不会乱序

https://blog.51cto.com/u_12827626/3318065

（由于重试机制导致的乱序）对于一个有着先后顺序的消息A、B，正常情况下应该是A先发送完成后再发送B，但是在异常情况下，在A发送失败的情况下，B发送成功，而A由于重试机制在B发送完成之后重试发送成功了。这时对于本身顺序为AB的消息顺序变成了BA。

针对这种问题，严格的顺序消费还需要`max.in.flight.requests.per.connection`参数的支持。

该参数指定了**生产者在收到服务器响应之前可以发送多少个消息**。它的值越高，就会占用越多的内存，同时也会提升吞吐量。把它设为1就可以保证消息是按照发送的顺序写入服务器的。

此外，对于某些业务场景，设置`max.in.flight.requests.per.connection`=1会严重降低吞吐量，如果放弃使用这种同步重试机制，则可以考虑在**消费端增加失败标记的记录，然后用定时任务轮询去重试这些失败的消息并做好监控报警**。

`max.in.flight.requests.per.connection`的意思是限制客户端在单个连接上能够发送的未响应请求的个数。**设置此值是1表示kafka broker在响应请求之前client不能再向同一个broker发送请求**，但吞吐量会下降

#### 前两个的总结

Kafka只能保证局部有序，即只能保证一个分区里的消息有序。而其具体实现是通过生产者为每个分区的消息维护一个发送队列，我们需要将保证顺序的消息都发送到同一个分区中。

并且由于Kafka会同时发送多个消息，所以还需指max.in.flight.requests.per.connection为1，保证前一个消息发送成功，后一个消息才开始发送。
...
+   生产者(Producer) 调用`send`方法发送消息之后，消息可能因为网络问题并没有发送过去。
    ...
    **可以采用为其添加回调函数**的形式（**检测到失败后，进行重试**），示例代码如下：
    ...
    另外这里推荐为 Producer 的`retries `（**重试次数）设置一个比较合理的值**，一般是 3 ，但是为了保证消息不丢失的话一般会设置比较大一点。设置完成之后，当出现网络问题之后能够自动重试消息发送，避免消息丢失。另外，建议还要**设置重试间隔**，因为间隔太小的话重试的效果就不明显了，网络波动一次你3次一下子就重试完了
    
    +   **设置异步发送，发送失败使用回调机制进行记录或重新发送**
    +   **失败重试，要设置重试次数**
...
**消费者默认是每隔5s提交一次offset。**

**当消费者拉取到了分区的某个消息之后**，**消费者会自动提交了 offset位移信息**。自动提交的话会有一个问题，试想一下，当消费者刚拿到这个消息准备进行真正消费的时候，突然挂掉了，消息实际上并没有被消费，但是 offset 却被自动提交了。
...
假如 leader 副本所在的 broker 突然挂掉，那么就要从 follower 副本重新选出⼀个leader ，但是 leader 的数据还有⼀些没有被 follower 副本的同步的话，就会造成消息丢失。

可以通过以下参数来配合：

**unclean.leader.election.enable=false**

该参数表示**不会选择，和 leader 同步程度达不到要求的follower副本，作为新的Leader** , 如果一个 Follower 的数据落后 Leader 太多，那么一旦它被选举为新的 Leader， 数据就会丢失，因此我们要将其设置为false，防止此类情况发生。

**replication.factor**

该参数**表示分区(partition)中副本的个数**。为了保证 leader 副本能有 follower 副本能同步消息,建议设置`replication.factor >=3`, 这样如果 Leader 副本异常 Crash 掉，Follower 副本会被选举为新的 Leader 副本继续提供服务。

**min.insync.replicas>1**

该参数**表示消息至少要被成功写入到多少个副本才算成功发送**，建议设置`min.insync.replicas > 1`，这样才可以提升消息持久性，保证数据不丢失。

另外还需要确保一下**replication.factor > min.insync.replicas**, 如果相等，只要有一个副本异常 Crash 掉，整个分区就无法正常工作了，因此推荐设置成：**replication.factor =min.insync.replicas +1**, 最大限度保证系统可用性。
...
-   Kafka 侧 由于服务端处理业务时间长或者网络链接等等原因让 Kafka 认为服务假死，触发了分区 rebalance（重平衡）。
    -   重平衡：一个Topic中的消息是由一个消费者组去进行消费的，而一个消费者组中存在多个消费者，每个消费者负责不同的分区，当其中一个消费者consumer2宕机后，该消费者负责的分区就被分配到消费者组内的其他消费者consumer1。这个重新分配的过程就是重平衡。
    -   由于消费者默认是每隔5s提交一次offset，此时已提交的offset和实际消费的offset不匹配
        -   如，已提交为3，实际消费到6，而consumer1还是从提交的offset--3开始消费，这样就会造成消息的重复消费
        -   如，当consumer2刚拿到这个消息准备进行真正消费的时候，突然挂掉了，消息实际上并没有被消费，但是 offset 却被自动提交了。这样就是消息的丢失 
...
-   将 
    ...
    `enable.auto.commit`
     ...
     参数设置为 false，**关闭自动提交**，开发者在代码中手动提交 offset。有同步提交、异步提交两种（通常采取两者结合的方式，先同步后异步）
...
-   消费消息服务做幂等校验，比如 Redis 的setNX、MySQL 的主键等天然的幂等功能。这种方法最有效。

#### 为什么选择异步而不是同步

kafka三种消息发送模式 https://blog.csdn.net/u010442302/article/details/106456474

+   异步
    +   发后即忘
    
        +   `kafkaTemplate.sent(topic, msg)`默认是异步发送，发后即忘
    ...
    +   异步发送消息时，只要消息积累达到batch.size值或者积累消息的时间超过linger.ms，producer就会把该批量的消息发送到topic中。
    
    +   kafka的异步模式可以让我们在业务场景中发送消息时即刻返回，不必等待发送的结果
...
+ 由于项目中的场景是：一旦涉及帖子的更新 / 修改操作（如点赞、评论），就会触发kafka的发送事件，这样一个高频的场景，批量处理的性能要比单个处理的性能要好，而且对数据的可靠性要求并不是很高，就算异步发送出错，kafka也还有重试机制
