面试官您好，我叫侯程元，来自湖南科技大学的计算机科学与技术专业，目前大四在读，专业成绩排名前20%，在校期间我自学了Java以及后端技术栈，像Spring、SpringBoot、关系型数据库MySQL、NoSQL数据库Redis，对JVM也有一定的了解。在成都的新希望六和公司有一段Java开发的实习经历，主要负责产融平台和供应链金融平台日常需求的开发和线上问题的排查。
...
-   否则，将该字符串添加到字符串池中，并返回对字符串池中的新引用
...
+   **连接过程**：HTTP 连接建立相对简单， TCP 三次握手之后便可进行 HTTP 的报文传输。
...
+ **短连接**，每次都需要进行一次TCP连接，而TCP的连接释放过程又是比较耗时的。
+ **队头阻塞**：由于HTTP1.0规定下一个请求必须在前一个请求响应到达之后才能发送，假设前一个请求响应一直不到达，那么下一个请求就不发送，后面的请求就阻塞了。

##### HTTP/1.1 相比 HTTP/1.0 优化了什么

- 使用**长连接**的方式减少了 TCP 连接的重复建立和断开所造成的额外开销，减轻了服务器端的负载
...
- **头部压缩**：HTTP/1.1 支持`Body`压缩，`Header`不支持压缩。HTTP/2.0 支持对`Header`压缩，使用了专门为`Header`压缩而设计的 HPACK 算法，减少了网络开销。**如果你同时发出多个请求，他们的头是一样的或是相似的，那么，协议会帮你消除重复的部分**。

- **多路复用**：HTTP/1.1 的实现是基于请求-响应模型的。同一个连接中，HTTP 完成一个事务（请求与响应），才能处理下一个事务，也就是说在发出请求等待响应的过程中，是没办法做其他事情的，如果响应迟迟不来，那么后续的请求是无法发送的，也造成了**队头阻塞**的问题。

	而 HTTP/2 就很牛逼了，**引出了 Stream 概念，多个 Stream 复用在一条 TCP 连接**

- **服务器推送**：HTTP/2 还在一定程度上改善了传统的「请求 - 应答」工作模式，服务端不再是被动地响应，可以**主动**向客户端发送消息。客户端和服务器**双方都可以建立 Stream**
...
HTTP/2 队头阻塞的问题是因为 TCP，是**基于 TCP** 实现的，于是**存在的缺陷有三个**：**TCP队头阻塞；TCP 与 TLS 的握手时延迟；网络迁移需要重新连接；**
...
是指在数据库设计中，有意地冗余部分数据以提高查询性能的一种技术。

如：student表中有stuname字段，score中也有stuname字段。

##### 优点

+   查询性能更高

　　冗余数据可以**避免连接多个表以获得所需数据，从而加快查询速度**。
...
+   当修改反范式表中的数据时，可能**需要在多个地方进行修改，这会导致更新异常**、更新开销大、数据不一致
...
https://learn.lianglianglee.com/%E4%B8%93%E6%A0%8F/MySQL%E5%AE%9E%E6%88%9845%E8%AE%B2/34%20%20%E5%88%B0%E5%BA%95%E5%8F%AF%E4%B8%8D%E5%8F%AF%E4%BB%A5%E4%BD%BF%E7%94%A8join%EF%BC%9F.md

什么是小表？

**在决定哪个表做驱动表的时候，应该是两个表按照各自的条件过滤，过滤完成之后，计算参与 join 的各个字段的总数据量，数据量小的那个表，就是“小表”，应该作为驱动表**

+   **Index Nested-Loop Join**（被驱动表上有可用的索引）

    **前提**：t1小表（100条），t2大表（1000条），t2的字段a有索引

    ```sql
    select * from t1 straight_join t2 on (t1.a=t2.a);
    ```
    
    先遍历表 t1，然后根据从表 t1 中取出的每行数据中的 a 值，去表 t2 中查找满足条件的记录。在形式上，这个过程就跟我们写程序时的嵌套查询类似，并且**可以用上被驱动表的索引**。
    
    **为什么小驱动大**：
    
    在这个 join 语句执行过程中，驱动表是走全表扫描，而被驱动表是走的树搜索
    ...
    **注意**：这里需要小驱动大的**前提是“可以使用被驱动表的索引“**
    
+   **BNL算法**（块嵌套循环连接）（被驱动表上没有可用的索引）

    **前提**：t1小表（100条），t2大表（1000条），被驱动表t2上没有可用的索引

    **算法流程：**
    
    1.  把表 t1 的数据读入线程内存 join_buffer 中，由于我们这个语句中写的是 select *，因此是把整个表 t1 放入了内存；
    2.  扫描表 t2，把**表 t2 中的每一行取出来，跟 join_buffer 中的数据做对比**，满足 join 条件的，作为结果集的一部分返回。
        1.  两个表都做一次全表扫描，所以总的扫描行数是 M+N；
        2.  内存中的判断次数是 M*N。
    
    引入了join buffer缓存区，**将驱动表缓存到join buffer中**（大小受join buffer的限制），然后全表扫描被驱动表，被驱动表的每一条记录一次性和join buffer中的所有驱动表记录进行匹配（内存中操作），将简单嵌套循环中的多次比较合并成一次，降低了被驱动表的访问频率。
    
    如果驱动表数据量较大，join buffer无法一次性加载驱动表的结果集，需要对被驱动表做多次全表扫描：
    
    1.  扫描表 t1，顺序读取数据行放入 join_buffer 中，假如放完第 88 行 join_buffer 满了，继续第 2 步；
    2.  扫描表 t2，把 t2 中的每一行取出来，跟 join_buffer 中的数据做对比，满足 join 条件的，作为结果集的一部分返回；
    3.  清空 join_buffer；
    4.  继续扫描表 t1，顺序读取最后的 12 行数据放入 join_buffer 中，继续执行第 2 步。
        1.  扫描行数是 N+λ*N*M；（K =λ*N，K为驱动表的分段数）
        2.  内存判断 N*M 次
...
+   SQL语句优化
    ...
    +   子查询优化
        ...
        +   **子查询改成连接查询**，子查询产生的临时表不可以用到索引，连接查询可以用到（看Index Nested-Loop Join）
    
        +   子查询产生的临时表是纯粹的中间表，用完就丢弃了；而连接查询产生的临时表（经过on过滤）是要最后返回给客户端展示用的。（子查询整个过程会产生两张临时表，连接查询产生一张）
    ...
    +   left join 或 right join 时要遵循小表驱动大表原则，inner join 会自动优化为小表驱动大表
        ...
        +   若被驱动表走不了索引，可以适当调大join buffer size
    ...
    +   深度分页：
        ...
        #优化2：适用于主键自增的表，把Limit查询转换成某个位置的查询，类似feed流，从上次分页查询的最小id开始
        SELECT * FROM student WHERE id > 2000000 LIMIT 100;
        ```
...
### 慢SQL如何定位
 ...
 - 运维工具skywalking可视化界面,查看到一些慢的接口,然后去找到sql进行explain的优化 	
 - 开启慢日志,设置阈值,去查看慢日志.找到sql并分析.但是在生产的环境下是不会开启慢日志的,很消耗性能!
...
+   **1、连接器**
    +   与客户端进行TCP三握手建立连接
        +   短连接
        +   长连接（推荐，减少建立连接和断开连接的过程但可能会导致内存占用增多）
...
+   解析器（**3、解析SQL**）
    ...
    +   语法分析：**根据词法分析的结果**，语法解析器会根据语法规则，**判断**你输入的这个 **SQL 语句是否满足 MySQL 语法**，**如果没问题就会构建出 SQL 语法树**，这样方便后面模块获取 SQL 类型、表名、字段名、 where 条件等等。
+   **4、执行SQL**
...
>   一个页的大小是16kb，用最坏的情况来假设一行记录大小为1kb，那么，一个叶子节点的页里就可以存放16条数据
>
>   在MySql数据库中，指针大小为6字节，若主键索引是bigint类型，那么就为8字节，两者加起来总共是14字节
>
>   一、先计算一个节点的字节大小：16kb * 1024 = 16384 字节。
>
>   二、16384 字节 / 14 字节 = 1170 ，意味着，根节点有1170个页地址指针，然后，每个页地址指针指向的叶子节点可以存放16条数据
>
>   三、那么三层B+树就是1170 * 1170 * 16 = 21902400，得出两千万左右条数据
>
...
https://blog.51cto.com/u_15077536/5955830

MySQL 8.0新特性，**当第一列索引的唯一值较少时（像性别、状态码这样的字段）**，即使where条件没有第一列索引，查询的时候也可以用到联合索引
...
![在这里插入图片描述](面试手册随笔.assets/bee2b3a7ba4e41ba9533c839d6d52cc1.webp)

总结：**索引跳跃扫描其实就是MySQL 8.0推出的适合联合索引左边列唯一值较少的情况的一种优化策略。**

#### 索引下推/索引条件下推

+   新版本的MySQL（5.6以上）中引入了索引下推的机制：**可以在索引遍历过程中，对索引中包含的字段先做判断，直接过滤掉不满足条件的记录，减少回表次数**。（**哪怕联合索引中只有第一条索引被使用到，剩下的索引列也可以被下推到**）https://xiaolincoding.com/mysql/base/how_select.html#%E6%89%A7%E8%A1%8C%E5%99%A8
...
可以在存储引擎层进行索引遍历过程中，**对索引中包含的字段先做判断**，直接过滤掉不满足条件的记录，再返还给 Server 层，从而减少回表次数

**针对联合索引的范围查询导致索引失效进行的一个优化**
...
+   **联合索引where后面的条件不满足最左匹配原则**
    ...
    +   ```sql
        ...
        是进行了索引覆盖，既不会回表，也不会索引下推，从a=2开始范围扫描。执行计划中key 为 idex_ab（联合索引），type 为 range，extra 为 Using where; Using index。
        ```
...
-   **range**（索引范围扫描）；
    ...
    -   **从这一级别开始，索引的作用会越来越明显，因此我们需要尽量让 SQL 查询可以使用到 range 这一级别及以上**
-   **ref**（非唯一索引扫描）；
    -   采用了**非唯一索引，或者是唯一索引的非唯一性前缀**，返回数据返回可能是多条。
    -   因为虽然使用了索引，但该索引列的值并不唯一，有重复。这样即使使用索引快速查找到了第一条数据，仍然不能停止，要进行目标值附近的小范围扫描。但它的好处是它并不需要扫全表，因为索引是有序的，即便有重复值，也是在一个非常小的范围内扫描。
-   **eq_ref**（唯一索引扫描）
    -   使用**主键或唯一索引**时产生的访问方式，通常使用在多表联查中
-   **const**（结果只有一条的主键或唯一索引扫描）
    -   使用了**主键或者唯一索引与常量值进行比较**，比如 select name from product where id=1。
-   const 类型和 eq_ref 都使用了主键或唯一索引，不过这两个类型有所区别，**const 是与常量进行比较，查询效率会更快，而 eq_ref 通常用于多表联查中**

##### index和All的区别

-   **All**（全表扫描）；扫描全部聚簇索引记录的方法；
-   **index**（全索引扫描）；
    -   如果key是主键索引，那么就是扫描聚簇索引树的所有索引值；
    -   如果key是二级索引，那就是扫描二级索引树的所有索引值。
...
主要是用来做可见性判断（**看看这个快照读能读到哪个版本的数据**），里面保存了 “**系统中当前不应该被本事务看到的其他事务 ID 列表**”。Read View就是事务进行快照读操作的时候生产的读视图(Read View)，在该事务执行的快照读的那一刻，会生成数据库系统当前的一个快照，记录并维护系统当前活跃事务的ID
...
+   当遇到唯一键冲突时，
    ...
    +   如果**唯一二级索引重复**，插入新记录的事务都会给已存在的二级索引列值重复的二级索引记录**添加 S 型 next-key 锁**
        ...
        +   如果不添加间隙锁的话，会让唯一二级索引中出现多条唯一二级索引列值相同的记录，这就违背了 UNIQUE 的约束

            如果只是记录锁， 假设这个唯一索引值是25, id是10, 你只锁住了二级索引中 '25, 10' 这个索引条目, 但是无法防止别人插入'25, xxx'这样的索引条目, 使用next-key lock, 可以锁住一个区间, 这样就可以防止别人重复插入相同的唯一索引值了
...
**bin log**：binlog是MySQL中server层次的日志，不管什么存储引擎都有binlog，他是一个数据库变更日志，他记录了像DML、DDL语句。他的作用大致有三种，1、主从复制中进行主从同步是基于binlog，2、备份恢复，3、还有在解决DB和缓存的一致性问题中，可以用cancel去监听binlog，以达到一致性的效果。

**redo log**：redo和undo都是属于InnoDB存储引擎下独有的。redo是记录了数据修改后的样子，他是用于实现ACID中的持久化，在数据库宕机后可以用来作为数据恢复。还有一点是，数据库中有个buffer pool，buffer pool 中有 undo 页，对 undo 页的修改也都会记录到 redo log，redo log 会每秒刷盘，提交事务时也会刷盘，数据页和 undo 页都是靠这个机制保证持久化的。

redo是以顺序、循环写的形式记录日志的，而binlog是追加写，写满一个文件，就创建一个新的文件继续写。

redo他是物理日志，记录的是对某某数据页的某某位置做了某某修改，binlog日志格式有三种statement(逻辑日志)、row、mixed

**undo log**：他是记录了数据被修改前的样子，然后执行反逻辑去恢复。有两个作用，1、保证原子性，通过回滚事务保证同时成功 / 失败

2、MVCC中用到了undo，MVCC中undo log形成了一条undo log版本链，当执行普通select语句读记录的时候，通过可见性算法去决定返回版本链中的哪个版本
...
undo log 和数据页的刷盘策略是一样的，都**需要通过 redo log 保证持久化**
...
-   **prepare 阶段**：将 XID（内部 XA 事务的 ID） 写入到 redo log，同时将 redo log 对应的事务状态设置为 prepare，然后将 redo log 刷新到硬盘；InnoDB持有prepare_commit_mutex
-   **commit 阶段**：把 XID 写入到 binlog，然后**先将 binlog 刷新到磁盘**，**再**接着调用引擎的提交事务接口，**将 redo log 状态设置为 commit**（设置为 commit 状态后，此时该状态标志并不需要持久化到磁盘，只需要 write 到文件系统的 page cache 中就够了，等系统调用fsync去刷入到磁盘）。写入COMMIT标记后释放**prepare_commit_mutex**
-   以 binlog 的写入与否作为事务提交成功与否的标志
...
通过日志保证数据的正确性（主要通过redo和bin，两阶段提交日志），而不是通过数据保证数据的正确性。

**日志是顺序写的，非常快。数据，是随机写的，数据如果非常分散，效率会低得一塌糊涂**。

简单点说：数据可以延后写，日志却不行。因为数据相对于日志，是根本不重要的。它并不决定数据的安全
...
+   List（quick List，双端链表，链表中的每个元素又是一个压缩列表）
    +   信息流展示
        +   最新文章、最新动态
...
+   Sorted Set（有序集合的元素个数小于 128 个，并且每个元素的值小于 64 字节时，Redis 会使用**压缩列表**作为 Zset 类型的底层数据结构；如果有序集合的元素不满足上面的条件，Redis 会使用**跳表**作为 Zset 类型的底层数据结构）
...
+   SDS：len（字符串长度）、alloc（分配的空间大小）、flags（sds类型）、buf[]（字符数组）
    ...
    +   **获取字符串长度的时间复杂度为O(1)**：SDS 结构加入了 len 成员变量
    ...
    +   **节省内存**，SDS 结构中有个 flags 成员变量，表示的是 SDS 类型。Redis 一共设计了 5 种类型，分别是 sdshdr5、sdshdr8、sdshdr16、sdshdr32 和 sdshdr64。这 5 种类型的主要**区别就在于，它们数据结构中的 len 和 alloc 成员变量的数据类型不同**。
...
+   SkipList：是对链表的一种改进。普通的链表在查找元素时，尤其是中间的元素时，需要逐一遍历，效率很低。
    ...
    +   为什么用跳表而不用平衡树
        +   **从实现难度上来比较，跳表比平衡树要简单得多**。平衡树的插入和删除操作可能引发子树的调整，逻辑复杂，而跳表的插入和删除只需要修改相邻节点的指针，操作简单又快速。（b+树同理）
    ...
    +   为什么不用b+
        +   https://blog.csdn.net/qq_43665821/article/details/126896149
        +   Redis 是内存数据库，进行读写数据时都是操作内存，跟磁盘无关，因此也不存在磁盘 I/O，所以层高就不再是跳表的劣势
        +   b+树本质上就是一颗**多路平衡二叉树**，当b+树中数据量较少的时候，添加新节点，可能会出现 拆分叶子节点、页分裂、叶子节点和索引节点都要拆分，同时还要往上再加一层索引  的情况，而跳表插入数据时，只需要经过随机函数，就能判断要不要往上加索引，根本不用考虑前后上下的节点，也就少了旋转平衡的开销
    +   查询过程
        +   1、从最高层点开始查询，若当前节点权重大于目标权重，则向下一层查找
        +   2、若等于目标权重，但当前节点的数据，若大于目标数据，则向下一层查找
        +   3、若当前的节点权重小于目标权重，则当前层的下一个节点查找
...
+   完全基于内存。Redis 绝大部分请求是纯粹的内存操作，非常快速。

+   redis底层有一个全局Hash表来维护所有的数据，查找和操作的时间复杂度都是 O(1)
...
+   Redis是单线程的（redis6后引入了多线程）。省去了很多**上下文切换**线程的时间，而且多线程必然会出现线程安全问题，因而必然**要使用锁去解决线程安全**。
    ...
    +   这些操作都是耗时间的，单线程处理的话会直接阻塞redis好长时间
...
目前的I/O多路复用都是采用的epoll模式实现

其中Redis的网络模型就是使用**I/O多路复用**结合**事件的处理器**来应对多个Socket请求，比如，提供了连接应答处理器、命令回复处理器，命令请求处理器

在Redis6.0之后，为了提升更好的性能，在**命令回复处理器**使用了**多线程**来处理回复事件，在命令请求处理器中，将**命令的转换**使用了**多线程**，增加命令转换速度，但是在命令执行的时候，依然是单线程
...
**Redis6.0** **引⼊多线程主要是为了提高网络** **IO** **读写性能**，因为这个算是 Redis 中的⼀个性能瓶颈（Redis 的瓶颈主要受限于内存和网络）

Redis6.0 引⼊了多线程，但是 Redis 的多线程只是在网络数据的读写这类耗时操作上使用了， 执行命令仍然是单线程顺序执行
...
![image-20230827234304437](面试手册随笔.assets/image-20230827234304437.png)
...
核心思路是Copy-On-Write。子进程刚产生时，此时子进程和父进程是共享同一片内存数据的，因为创建子进程的时候，会复制父进程的页表，但是页表指向的物理内存还是一个
...
+   **不会阻塞当前的写操作**，因为当写操作命令执行成功后，才会将命令记录到 AOF 日志。**可能会给下一个命令带来阻塞风险**，损人利己
+   对比MySQL：

    +   MySQL实现了这个WAL（**预写式日志（Write-Ahead Logging**）技术，来保证原子性和持久性。两个数据库的侧重点不同，MySQL更侧重于持久化、保障数据可恢复，Redis更侧重于高性能读写
...
![image-20230907204758661](面试手册随笔.assets/image-20230907204758661.png)

##### 增量同步

![image-20230923233359975](面试手册随笔.assets/image-20230923233359975.png)
...
如果主节点或者从节点没有在规定的时间内响应哨兵的 PING 命令，这个哨兵就会将它们标记为「**主观下线**」，然后开始投票，当赞成票大于二分之一时，这个哨兵就会标记为 客观下线（客观下线只适用于主节点） ，这时就开始选举新的leader
...
-   第一步：在已下线主节点（旧主节点）属下的所有「从节点」里面，挑选出一个从节点，并将其转换为主节点。（判断从节点的slave-priority值，越小优先级越高；offset值越大优先级越高；从节点id越小优先级越高）
...
![image-20230907223446869](面试手册随笔.assets/image-20230907223446869.png)

#### 切片集群

控制了单个节点的数据量,但是又要应对海量数据和高读并发,又该如何处理?

![image-20230924111307472](面试手册随笔.assets/image-20230924111307472.png)

redis的分片集群解决了两个问题:
			(1)真正的海量数据的分布式存储
			(2)主从解决了高并发读的问题，并没有解决高并发写的问题
...
+   三次Hash映射到数组上，判断其是否在数组中，这个hash个数是动态改变的
...
+   解决方法
    +   **互斥锁：**当key过期时，使用redis设置一个互斥锁，进行缓存重建。
    +   **互斥锁+逻辑过期**（逻辑过期不需要考虑缓存穿透问题,因为所有的数据已经存入redis中预热了,一旦缓存查出是null,说明数据库中没有该数据）当访问到key的时候，就去判断value中的时间是否过期，new一个线程去负责重建缓存，其余线程读到的就是旧数据
...
+   解决了并发，继续**看操作失败问题**。可以采用重试（两种方案本质都是重试），**简单点，直接先写db后删缓存，失败就起协程时间分级异步重试**，MQ和binlog逻辑太重，又引入了新的组件，引入的越多，只会使一致性越来越难达成
    ...
    +   **消息队列**：就是把重试请求写到「消息队列」中，然后由专门的消费者来重试，直到成功。
        ...
        或者更直接的做法，为了避免第二步执行失败，我们可以把操作缓存这一步，直接放到消息队列中，由消费者来操作缓存（**redis的一致性策略就是允许短暂的读到未修改的数据**，在MQ没消费之前，允许读到旧数据）
...
+   总结：
    ...
    +   要想做到强一致性，可以通过加「分布锁」的方式来实现，但我们要付出的代价，很可能会超过引入缓存带来的性能提升
...
<img src="面试手册随笔.assets/image-20230824083852046.png" alt="image-20230824083852046" style="zoom:80%;" />

##### 热点key

###### 产生原因

微博热点新闻、京东618抢购；请求分片集中，超过单server的性能极限

###### 危害

流量集中，达到物理网卡上线；请求过多，缓存分片服务被打垮；DB击穿，引起业务雪崩

###### 发现热点key

+   预估发现
    +   提前与客户沟通活动的支撑量和细节；凭借经验
+   客户端发现
    +   客户端做一个本地内存，对每一个请求数据做Map<数据，次数>统计
    +   无法预知key的个数，存在内存泄露风险
    +   对客户端代码有侵入
+   redis发现
    +   monitor命令：可以监控到redis执行的所有命令
    +   redis -cli --hotkeys
...
https://www.bilibili.com/video/BV1Us4y167cN 10:14
...
![image-20230824173701983](面试手册随笔.assets/image-20230824173701983.png)
...
+   ![image-20230824173725087](面试手册随笔.assets/image-20230824173725087.png)

**JDBC解决线程安全问题**

https://blog.csdn.net/u010833547/article/details/106467621

那么负责数据库连接的Connection也只有一个， 如果每个请求线程都去连接数据库，共用一个connection，那么就会造成线程不安全的问题，Spring是如何解决这个问题的呢？
...
https://www.bilibili.com/video/BV1B14y1B73v 13.20
...
+   **验证**：**确保Class文件的字节码文件中包含的信息符合当前虚拟机的规范，是否包含像魔数这样的信息，是否会危害虚拟机自身的安全**
...
+   **准备**：为**类的静态变量**分配内存（在方法区中分配内存），并将其初始化为默认值（数据类型默认的零值，而不是代码中显式赋予的值）
    ...
    +   对于static final 修饰的变量，在编译阶段就已经完成了内存的分配，此时会对该变量进行初始化
    
+   **解析**：**把静态常量池中的符号引用转换为运行时常量池的直接引用**

    ​			类的字节码文件里面有静态常量池，里面存储了一些字面值和符号引用，这时类已经确定了，确定之后会把符号引用转化为方法区中运行时常量池的直接引用
...
JVM 开始从**入口方法（main）**开始执行用户的程序代码（main方法用static的原因：不是静态，jvm要新建对象才能调main方法）
...
+   **通过SPI机制**，使用`ServiceLoader.load`去加载
    ...
    +   JDBC破坏双亲委派：
        ...
        **Driver 这类接口，他是属于BootStrap ClassLoader加载的，但是他的实现类是不同的数据库厂家提供的，BootStrap ClassLoader加载不到这些类，所以使用了DriverManager利用SPI机制获取接口的实现，SPI中直接指定让 app ClassLoader 去加载**
...
+   Tomcat
	...
	Tomcat打破双亲委派机制的目的其实很简单，我们知道**web容器可能是需要部署多个应用程序**的。但是假设**不同的应用程序可能会同时依赖第三方类库的不同版本**。我们通过上文在了解类加载机制的时候知道它是要确保唯一性的，但是总**不能要求同一个类库在web容器中只有一份**吧？所以Tomcat就需要**保证每个应用程序的类库都是相互隔离并独立的**，这也是它为什么打破双亲委派机制的主要目的。

	<img src="面试手册随笔.assets/fed949cb56d54f02e9ac800a8cefca3b.png" alt="img" style="zoom:50%;" />
	...
	Tomcat的设计是违背Java的双亲委派模型的，每个`WebappClassLoader`加载自己目录下的.class文件，**不会传递给父加载器，这就打破了双亲委派机制**，这样做正是为了实现隔离性
...
发生时机：当一个对象在方法中被定义后，它被外部方法引用，则认为发生逃逸；否则，没有发生逃逸

没有发生逃逸的对象，可以分配到栈上

hotsport没有采用逃逸分析（需要复杂分析，比较耗时）
...
+ JDK8默认 垃圾回收Q器: ParallelGC + ParallelOldGC
+ JDK9默认垃圾回收器: G1
+ JDK14默认的垃圾回收器: G1
...
由于在垃圾收集阶段用户线程还需要持续运行，那就还需要预留足够内存空间提供给用户线程使用，因此CMS收集器不能像其他收集器那样等待到老年代几乎完全被填满了再进行收集，必须预留一部分空间供并发收集时的程序运作使用，JDK5默认CMS收集器当老年代使用了68%的空间后就会被激活；JDK6，提升到了92%
...
G1基于“停顿时间模型”而建立的垃圾收集器，停顿时间模型意思是支持在M毫秒的时间片顿内，消耗在垃圾收集上的时间大概率不超过N毫秒。（默认是200毫秒，也可由用户指定）
...
G1仍然保留新生代和老年代的概念，但**新生代和老年代不再是固定的了，它们都是一系列区域（不需要连续）的动态集合**，还有humongous专门存储大对象。

Eden区大概占5%~6%

young gc（新生代回收STW）、并发标记（老年代占用内存超过阈值，默认45%）+重新标记、mixed gc（混合回收），如果回收速度赶不上创建新对象的速度，就full gc
...
增量更新：当黑色对象插入新的指向白色对象的引用关系时，就将这个新插入的引用记录下来，等并发扫描结束之后，再将这些记录过的引用关系中的黑色对象为根，重新扫描一次（相当于黑色对象又变成了灰色对象）
...
G1和CMS的解决方案是对同一个漏标现象的不同角度去处理的

#### G1和CMS适用场景，G1为什么内存占用比CMS多

大内存G1，小内存cms

因为G1需要维护一些表，记录那些回收价值大的region。需要维护 对于每一个region都需要记录，以实现他对于垃圾回收时间的可控性
...
+   大访问压力下，会导致对象的大量创建，可以适当增大一些Eden区的空间，这样可以降低YGC的频率，不过这样就会因为空间增大，STW时间也增长，因此要从实际考量
...
+   **弱引用**
    ...
    +   WeakReference
    +   它比软引用的生存期更短，对于只有弱引用的对象来说，**只要垃圾回收机制一运行，不管 JVM 当前的内存空间是否足够，总会回收该对象占用的内存。**
...
Java中的unsafe类，提供了CompareAndSwapInt / Long / Object的native方法 ，这些方法都是直接操作内存的原子操作，它们可以实现对变量的原子修改，底层是一个CPU的原子指令-->cmpxchag，对总线加锁保证原子性
...
monitor（c++实现）：WaitSet（wait / notify）、EntryList（阻塞队列）、owner（指向持有Monitor对象的线程）、recursions（重入次数）
...
+   偏向锁
    ...
    +   **核心思想**是：如果一个线程获得了锁，那么锁就进入偏向模式，此时Mark Word 的结构也变为偏向锁结构，当这个线程再次请求锁时，只要检查MarkWord中是不是存储着当前线程的ID，即可获取锁
        ...
        第一次加偏向锁要cas线程id后面锁重入发现线程ID就是自己，就直接进了
...
+   `Terminated`：终止态，表示该线程已经运行完毕。执行完run方法
+   <img src="面试手册随笔.assets/image-20230409184133909.png" alt="image-20230409184133909" style="zoom: 67%;" />

![image-20230826233059875](面试手册随笔.assets/image-20230826233059875.png)
...
写时复制、不可变、redis单线程、ThreadLocal、锁互斥

### JMM

由于底层CPU存在缓存不一致的情况，因此JVM在这一层做了一系列的工作，来屏蔽各种硬件和操作系统的内存访问差异，以实现让Java程序在各种平台下都能达到一致的内存访问效果。

Java内存模型规定了所有的变量都存储在主内存中（类比物理硬件的内存）。每条线程还有自己的工作内存（可与处理器高速缓存和寄存器类比），线程的工作内存中保存了被该线程使用的变量的主内存副本，线程对变量的所有操作（读取、赋值等）都必须在工作内存中进行，而不能直接读写主内存中的数据。
...
+   线程切换带来的**原子性**问题。典型：i++
	...
	+   由JMM来直接保证的原子性变量操作包括read、load、assign、use、store和write这六个，我们大致可以认为，**基本数据类型的访问、读写都是具备原子性的**
	    ...
	    但是如果想要粒度大一些的话
	    考虑上锁的原子性操作lock和unlock，但虚拟机并未直接将指令暴露给用户，而是用了更高级别的字节码指令monitorenter和monitorexit来隐式的实现这两个指令，该字节码指令反映到Java代码中的就是synchronized关键字了!
...
+   编译优化带来的**有序性**问题？？？如果在本线程内观察，所有的操作都是有序的；如果在一个线程中观察另一个线程，所有的操作都是无序的
    ...
    +   synchronized则是由“一个变量在同一个时刻只允许一条线程对其进行lock操作”这条规则获得的，这个规则决定了持有同一个锁的两个同步块只能串行地进入，**并没有禁止指令重排序，严格来说没有解决有序性问题**
...
https://mp.weixin.qq.com/s/easnVQ75Rq-C07W4YWeclQ  操作系统层面解释

答案是肯定的，线程崩溃的确会造成进程崩溃。

**因为所有线程共享进程地址空间，线程没有自己单独的内存地址空间，他们之间是完全透明的**，一个线程崩溃就说明该线程此时已经出现了不可逆转的错误，该错误**极有可能已经破坏掉了或者即将破坏掉进程的地址空间**，因此在操作系统眼里看来继续运行其它线程是有很大风险的，因为大家共同运行的基础已经没有了(完好的地址空间)，这就是典型的**一粒老鼠屎坏了一锅粥。**

**只要一个线程崩溃，那么在操作系统眼里看来所有其它线程已然不可信，因此操作系统手起刀落**
...
虚拟机内部定义了信号处理函数，而在信号处理函数中对信号做了额外的处理以让 JVM 不崩溃，抛给用户异常 / 错误
...
+   锁与同步
+   `wait()` / `notify()` 或 `notifyAll()`   park / unpark  await / signal
+   信号量
...
是java里的关键字，一般是配合CAS 作为Java中的无锁并发编程来使用。volatile是保证了可见性和有序性（禁止指令重排序），能保证单条JVM字节码指令的原子性（保证单次的读/写操作具有原子性），应用的话是JUC中的原子整数、原子累加器、AQS，都是基于CAS+volatile的方式实现的。
...
CPU最小的存储单位是缓存行，volatile修饰的变量和普通变量都会存放到缓存行里，如果我们仅仅是想要读取volatile修饰变量的值，直接从缓存行中读取即可，无需考虑内存的可见性；volatile的可见性主要发生在多核并发，对于volatile修饰变量的读写操作上，多线程环境下，如果一条线程对volatile的变量进行了写操作，该写操作会直接投过总线推送到主存，同时使其他CPU的缓存行无效，这时另外一个线程想要读取该值的时候，发现当前CPU的缓存行无效，就会从总线重新拉取该值。

在 volatile 修饰的共享变量进行写操作的时候在汇编层面，写操作的指令会多出 lock 前缀指令 

Lock前缀指令的作用：

+   将当前处理器缓存行的数据写回到系统内存。
+   写回内存的操作会使在其他 CPU 里缓存了该内存地址的数据无效（当处理器发现本地缓存失效后，就会从内存中重读该变量数据，即可以获取当前最新值）
+   配合总线上的缓存一致性协议(MESI)，实现了可见性

为了提高处理速度，CPU不直接和内存进行通信，而是先将系统内存的数据读到内部缓存(L1，L2 或其他)后再进行操作，但操作完不知道何时会写到内存。

如果对声明了 volatile 的变量进行写操作，JVM 就会向处理器发送一条 lock 前缀的指令，将这个变量所在缓存行的数据写回到系统内存。Lock有两个作用：

+   将我们当前的修改立即透过总线推送到主存
+   在透过总线时，由于总线的嗅探技术（对于Lock前缀的一个嗅探），会导致其他CPU的缓存行无效，当处理器发现本地缓存失效后，就会从主存中重读该变量
...
**3.volatile修饰的变量在底层汇编层面，当发生修改时会加上lock前缀；**

**在这三个条件的配合下，才能够最终的达到volatile的一个可见性**
...
+   作用
    ...
    +   synchronized 可以保证可见性、原子性、有序性（一个变量在同一个时刻只允许一条线程对其进行lock操作）
...
#### AQS和 CAS，为什么要 CPU 轮询而不是阻塞挂起等通知

挂起会有线程上下文切换，阻塞是用户态和内核态切换

AQS就是用cas避免借助操作系统的线程阻塞。直接从java层面实现锁

### AQS

https://www.bilibili.com/video/BV1Lq4y147k5  17:59

AbstractQueuedSynchronizer，抽象队列同步器。

是一个抽象类，就是在Java中去实现锁、同步器的一个核心组件。他内部有一个被volatile修饰的共享变量**state**，state在不同实现中有不同的含义，操作state就是通过CAS的方式。还有一个是**阻塞 / 同步队列**，基于 **FIFO 的双向链表**实现的，暂时获取不到锁的线程将被加入到该队列中。还有一个**条件 / 等待队列**，他是个**单向链表**。相当于Java中wait-notify机制，在ReentrantLock就是await-signal，当获取到锁以后，发现条件不满足，这时调用await，就会去条件队列中等待。还有，AQS继承了**AbstractOwnerableSynchronizer**，AQS自己维护了一个owner，这个owner就是锁的持有者，记录着锁被哪个线程获取到了，是Thread类的一个封装。

调用await其实就是把同步队列的首节点加到条件队列的尾部**再**释放锁，因此这个过程不需要CAS避免并发冲突，而**加入阻塞队列需要CAS**，多个线程竞争锁失败加入阻塞队列的尾部
...
同步器的设计是基于**模板方法模式**的，也就是说，**使用者需要继承同步器并重写指定的方法**，随后将同步器组合在自定义同步组件的实现中，并调用同步器提供的**模板方法**，而这些模板方法将会调用使用者重写的方法。
...
在加入AQS同步队列的尾部时，这个时候存在并发问题（既然要加入到同步队列的尾部，就意味着竞争锁失败了，也就意味着此时可能有多个线程都竞争失败了），多个线程竞争锁失败加入同步队列的尾部，通过**CAS解决**；当前持有锁的线程释放锁后，通知自己的后继结点去竞争锁；
...
1.   lock()方法，ReentrantLock中的lock实际上是调用了同步器的lock，sync.lock();

2.   unlock()方法，sync.release()---》tryRelease()

3.   tryLock()方法：非阻塞进行加锁（不等待，先做其他事情）,不阻塞意味着少一次从用户态到内核态的切换

4.   tryLock(long time, TimeUnit unit)：超时性加锁

5.   lockInterruptibly()：可中断式加锁

6.   newCondition()：完成消息通知等待机制

### ReentrantLock

https://zhuanlan.zhihu.com/p/82992473

<img src="面试手册随笔.assets/image-20230828201841391.png" alt="image-20230828201841391" style="zoom:50%;" />
...
+   **公平锁**：**锁释放后，会先判断 AQS 等待队列中是存在节点，如果存在节点则会直接入队等待**。性能较差一些，因为公平锁为了保证时间上的绝对顺序，上下文切换更频繁。

    +   与非公平锁主要区别在于 tyAcquire 方法:先检查 AQS 队列中是否有节点，没有才去 CAS 竞争
...
+   **非公平锁**：锁被释放后，后申请的线程可能会获取到锁，是随机或者按照其他优先级排序的。性能更好，但可能导致某些线程永远无法获得到锁
    ...
    非公平锁提高效率的原因，（**减少CPU唤醒线程的开销**）主要是因为**插队的线程不需要进入等待队列，从而避免了两次上下文切换**（用户态和内核态之间的切换，主要是保存和还原cpu寄存器等相关的值），降低了资源开销、节约了时间。
...
#### 什么情况用线程池，什么情况用多个线程而不用线程池

线程池：单个任务处理时间短、需要处理的任务数量大、需要控制或限制最大并发线程数量

多个线程：

+   任务执行时间很长，几分钟或更久的任务，长时间占用线程资源效率不高。
+   任务结束后需要完成一些线程私有的清理操作
+   任务起止时机不确定,难以事先提交到线程池
...
看这个：当线程数大于核心线程数，且阻塞队列已满，这时候就会**创建非核心线程，同时设置超时标志为true，这个超时标志为true时，就会从执行一个带有超时时间（keepAliveTime）的代码，超时时间内未获取到任务就返回null，然后销毁当前线程**。
...
**shutdown** / **shutdownNow**方法来关闭线程池。它们的原理是**遍历线程池中的工作线程，然后逐个调用线程的interrupt方法来中断线程**，所以无法响应中断的任务可能永远无法终止。
...
+   使用 isTerminated 方法判断。
    ...
    +   exector.isTerminated()，前提是需要将线程池关闭

+   使用 getCompletedTaskCount 方法判断

    +   如果计划执行任务数==已完成任务数，那么线程池的任务就全部执行完了
    ...
    +   ```java
        ...
        void isCompletedByTaskCount(ThreadPoolExecutor threadPool) {
...
+   使用 CountDownLatch / CyclicBarrier 判断

    +   countDownLatch.countDown();、countDownLatch.await();

+   如果是带有返回值的任务，submit后可以得到Future类型的返回值。通过返回值可以判断线程任务的执行状态。

    future.isDone()

    +   如果是提交了多个任务，可以收集所有的Future到集合中，轮询这个集合
...
当任务出现未被捕获到的异常时，会将**执行该任务的线程从线程池移除并结束掉，然后移除之后创建一个新的线程放回到线程池中**。

线程池内部会 try - catch 去捕获异常，但在catch中直接将异常抛出。

可以通过ThreadFactory自定义线程并捕获线程内抛出的异常，也就是说甭管我们是否去捕获和处理线程池中工作线程抛出的异常，这个线程都会从线程池中被移除。

通过ThreadFactory自定义线程来捕获异常：
...
如果使用的是submit(Callable<T>)，可以利用返回值future的get来获取任务执行结果，若执行过程中出现异常，会抛出ExecutionException，其中包含了实际的异常
...
**联系monitor对象**，使用Thread的话，无法对锁对象关联的monitor对象进行操作，不知道锁的是哪个，唤醒的是哪个
...
+   减少内存开销。假如使用 ReentrantLock ，那么每个节点都需要通过继承AQS 来获得同步支持。但实际上并不是所有的节点都需要获得同步支持，只有头结点需要，这样就增加了内存开销
    ...
    ```java
    //1.7 Segment extends ReentrantLock
...
ioc的思想最核心的地方在于，**资源不由使用资源的双方管理，而由不使用资源的第三方管理**，这可以带来很多好处。第一，资源集中管理，实现资源的可配置和易管理。第二，降低了使用资源双方的依赖程度，也就是我们说的耦合度。（**支付宝**）

IOC（控制反转）是一种设计思想，就是 **将原本在程序中手动创建对象的控制权，交给Spring框架来管理**， 当某个对象需要其他协作对象时，由Spring动态的通过依赖注入(DI)的方式来提供协作对象。
...
**控制反转IOC 就是依赖倒置原则DIP的一种代码设计的思路。具体采用的方法就是所谓的依赖注入DI**

依赖倒置：是指设计代码结构时，**高层模块不应该依赖低层模块，二者都应该依赖其抽象**https://blog.csdn.net/gupaoedu_tom/article/details/120905135
...
+   AspectJ AOP
    ...
    +   **基于字节码操作**
+   Spring AOP
    +   **使用的是动态织入**，这种方式是在运行时动态地将要增强的代码织入目标类中，这样往往是通过动态代理技术完成的。运行时临时为方法生成一个AOP代理对象，该代理对象包含了目标对象的全部方法，并在特定的切点做了增强处理，并回调原对象的方法。
    +   **基于代理**
+   如果我们的切面比较少，那么两者性能差异不大。但是，**当切面太多的话，最好选择 AspectJ** ，它比 Spring AOP 快很多（动态代理依赖于反射机制，运行时动态进行；AspectJ是字节码操作，编译期间进行）

#### JDK代理和CGlib代理的区别

1.jdk动态代理和cglib动态代理的区别:
			jdk动态代理是java实现动态代理的原生态方式，而cglib是第三方工具库.是对jdk动态代理的补充!
			Spring对普通类使用cglib的方式来实现代理，对实现了接口的类通过jdk动态代理。
		2.为什么jdk动态代理代理的目标类都必须实现接口呢?
			jdk动态代理在实现上有限制，它只能针对实现自某接口的类，在生成代理类的时候,该代理类是实现自proxy类的，并不是继承目标类的。但java中只支持单继承，所以不能继承目标类了，只能实现!
...
+   **第二步：依赖注入，bean属性赋值，set方法注入**
    +   $\textcolor{blue}{第三步：before方法之前，检查Bean是否实现了Aware的相关接口，并调用方法，目的是为了传递一些Bean相关的数据，方便使用}$（结合点评项目中mq的配置类，实现了ApplicationContextAware 接口，在setApplicationContext中获取rabbitTemplate，设置回调函数，对消息发送失败的情况进行了处理，将失败的消息存入DB消息表 / 重新发送到队列）
...
+   **第六步：初始化bean，调用bean的init方法，此方法需要自己写，自己配**
    +   $\textcolor{red}{第七步：在初始化bean之后，执行bean后处理器（BeanPostProcessor）的after方法，后置处理}$
        ...
        （AOP，这一步是是Spring给我们提供的扩展点，可以拿到可用的原始对象，我们的代理对象生成和替换就是在这里）
    
+   **第八步：使用bean**
    
    +   $\textcolor{blue}{第九步：销毁Bean之前，检查Bean是否实现了DisposeableBean接口，Spring容器会调用这个接口中的destroy方法来进行对象的销毁}$
...
- 在Spring的单例模式的Bean中不应该在全局层面上定义有数据存储功能的对象 / 基本类型成员变量 / static变量，会产生线程安全问题；若要这么做，可以通过声明Bean的 Scope=‘prototype’ 或者使用本地变量 ThreadLocal 进行定义
...
一.简单工厂模式
beanFactory的getBean(),通过唯一标识来获取bean对象,是典型的简单工厂模式的体现

二.工厂方法模式
通过factoryBean接口来进行bean的加工创建,协助Spring创建Bean.比如: 整合mybatis时创建的SqlSessionFactory大对象就是通过
SqlSessionFactoryBean实现了FactoryBean接口来创建的! 是工厂方法模式的体现

三.单例模式
Spirng使用的是双重锁机制实现单例模式.
在依赖注入时,如果判断出现了循环依赖的时候,优先从一级缓存中拿,然后依次从二级 三级缓存中拿值.在getSingleton()方法中

四.代理模式
典型的AOP就是代理模式的体现.(动态代理)

五.装饰者模式
比如: IO流中BufferedReader缓冲流,和里面包括的流."节点流"和"包装流"体现着装饰者模式,装饰者可以对被装饰者
  进行装饰 进行功能增强.
Spring中配置dataSource数据源时,数据源可能是不同的,如何在少修改原有代码的前提下,动态切换不同的数据源.
这里就是装饰器模式的体现.
Spring中带有Decorator和Warpper单词的类都是修饰器模式的体现

六.观察者模式
典型的有web中的listener监听器,当状态发生改变会执行一些方法等...
Spring的事件编程模型就是观察者模式的体现.Spring中定义了一个ApplicationListener来监听容器Application
的事件,事件发生会执行一些方法.

七.策略模式
策略模式重点体现就是面向接口编程,降低耦合度,提高程序扩展力.
这是一种行为性模式,根据调用方法的不同,适应行为的变化.强调父类调用子类的特性.
getHandler()根据请求通过"处理器映射器"获取到该请求对应的处理器执行链.
比如一个dao接口,我实现了多个实现类.在service层调用时不需要关心接口的的底层
具体实现,只需要享受服务,面向接口调用.底层可以灵活切换,这就是策略模式.

补充: 线程池中拒绝策略是一种策略模式体现,RejectExecutionException是接口,然后具体采用哪种拒绝策略,由用户来传入!

八.模板方法设计模式
具体实现在子类中完成,父类只是作为模板类和模板方法定义一些核心的东西!
Spring中JdbcTemplate类就是一个模板类,是模板方法设计模式的体现.
在模板类的模板方法中写一些核心的操作,程序员在子类中进行具体的编码来达到功能.
...
+   一级：singletonObjects，ConcurrentHashMap，单例池，**缓存已经经历了完整的Bean生命周期，已经初始化完成的Bean对象**
+   二级：earlySingletonObjects，HashMap，**完成实例化，但是还未进行属性注入及初始化的对象**（半成品，生命周期还未走完）
+   三级：singletonFactories，HashMap**提前暴露的一个单例工厂，二级缓存中存储的就是从这个工厂中获取到的对象**
...
![image-20230829232339108](面试手册随笔.assets/image-20230829232339108.png)

ObjectFactory对象的作用：如果原始对象是代理对象，则生成一个代理对象，否则生成一个普通对象，直接注入给 B

在此处的**二级缓存**看似可以不存在，实际上将代理对象A（半成品）存入二级缓存，在后续 注入给B 和 将B注入给A 这两步中，都需要从二级缓存中去获取代理对象A，否则每次都调用ObjectFactory去生成，会产生多例。**保证单例性**

#### 只能解决单例+setter注入的模式

+   构造器注入

    +   beanB需要在beanA的构造函数中完成初始化，beanA也需要在beanB的构造函数中完成初始化，这种情况的结果就是两个bean都不能完成初始化，循环依赖难以解决。**三级缓存解决的是初始化过程中的循环依赖，构造器注入还没进入初始化的过程中**
...
其实只用两级缓存就可以基本解决循环依赖（singletonObjects+earlySingletonObjects），但是两级缓存无法解决循环依赖中有代理对象去情况。

之所以**不能解决代理对象的情况，关键在于创建代理对象的时机晚于依赖注入的时机**。

b在进行依赖注入的时候，注入的是earlySingletonObjects中 a 的原始半成品对象，并不是我们需要的代理对象
...
+   异常捕获处理，自己处理的异常（try-catch），但是没有抛出
...
| :----------------------------: | :----------------------------------------------------------: |
| ISOLATION_READ_**UNCOMMITTED** |   允许读取尚未提交的更改。可能导致脏读、幻读或不可重复读。   |
|  ISOLATION_READ_**COMMITTED**  | （Oracle 默认级别）允许从已经提交的并发事务读取。可防止脏读，但幻读和不可重复读仍可能会发生。 |
| ISOLATION_**REPEATABLE_READ**  | （MYSQL默认级别）对相同字段的多次读取的结果是一致的，除非数据被当前事务本身改变。可防止脏读和不可重复读，但幻读仍可能发生。 |
|   ISOLATION_**SERIALIZABLE**   | 完全服从ACID的隔离级别，确保不发生脏读、不可重复读和幻影读。这在所有隔离级别中也是最慢的，因为它通常是通过完全锁定当前事务所涉及的数据表来完成的。 |
...
https://www.bilibili.com/video/BV1e14y1A7pT

添加注解后，通过运行SpringApplication的run方法完成服务构建、环境准备、容器创建、填充容器四项工作。
...
+   `${}`：是静态文本替换。执行SQL时会**将参数直接拼接到SQL语句中**，**参数不会进行转义 并 以原值传入**，即传入的参数不会加上单引号。**底层使用的是statement 。因此无法防止SQL注入**（字符串需要手动加引号）
...
+   `#{}`：是SQL的参数占位符，执行SQL时会将SQL中的`#{}`替换为 `? `，在**SQL执行前**使用`PreparedStatement `，按序给SQL的占位符设置参数值。（利用反射机制）因此 # 可以防止SQL注入提高安全性

    执行SQL时会将其替换为 ? 进行占位参数会进行转义，并以字符串传入，即传入的参数会自动加上单引号。底层使用的是Preparedstatement 。
    
    **自动添加双引号**
    
+   SQL注入是 攻击者通过在应用程序中注入恶意代码来执行 **未经授权的SQL查询**，并可能访问、修改或删除数据库中的敏感数据

    +   是安全的，因为这种方式会对输入的值进行预处理和转义，从而避免了SOL注入攻击
    
    +   $ 可能存在SOL注入问题，因为它不会对输入的值进行转义和预处理，而是直接将输入的值嵌入到SOL语句中
...
- 简单模式
	...
	- ![image-20230913184131189](面试手册随笔.assets/image-20230913184131189.png)
- work 工作模式
	- 一个消息生产者，一个交换器，一个消息队列，多个消费者。也称为点对点模式
	- 工作队列也称为**公平性队列模式，**循环分发，RabbitMQ 将按顺序将每条消息发送给下一个消费者，每个消费者将获得相同数量的消息。
	- ![image-20230913184149711](面试手册随笔.assets/image-20230913184149711.png)
- pub/sub 发布订阅模式
	- 需要**设置交换机类型为 fanout**(广播),并且交换机和队列进行绑定,当发送消息到交换机后,交换机会将消息发送到绑定的队列,不用指定 routingKey(空字符串) ,消息和队列是一对多的关系,一条消息可以被多个队列消费
	- ![image-20230913184301459](面试手册随笔.assets/image-20230913184301459.png)
- Routing 路由模式
	- 需要**设置交换机的类型为 direct**(定向),交换机和队列进行绑定并且指定 routingKey(路由键),当发送消息到交换机后,交换机会根据 routingKey 将消息发送到对应的队列, 消息和队列是一对多的关系,一条消息可以被多个队列消费
	- 多个消息队列，多个消费者
	- ![image-20230913184500337](面试手册随笔.assets/image-20230913184500337.png)
- Topic 主题模式
	- topics(主题)模式跟routing路由模式类似，只不过路由模式是指定固定的路由键 routingKey，而主题模式是可以模糊匹配路由键 routingKey，类似于SQL中 = 和 like 的关系。
	- ![image-20230913184526450](面试手册随笔.assets/image-20230913184526450.png)
...
1.  `生产者丢失`：生产者将数据发送到RabbitMQ的时候，由于网络的原因，可能数据半路就丢了。此时可以选择使用RabbitMQ提供的事务功能，就是生产者发送数据之前开启RabbitMQ事务channel.txSelect，然后发送消息，如果消息没有被RabbitMQ接收到，那么生产者会收到异常报错，此时可以回滚事务channel.txRollback，然后重试发送消息；如果收到了消息，那么可以提交事务channel.txCommit。吞吐量会下来，因为太耗性能。
    ...
    MQ提供了publisher confirm机制来避免消息发送到MQ过程中丢失。消息发送到MQ后，会返回一个结构给客户端，表示消息是否处理成功。结果有两种请求：

    +   publisher-confirm，发生者确认   new CorrelationData(**id**.toString()).getFuture().addCallback
        +   消息成功投递到交换机，返回ACK
            +   channel.basicAck
        +   消息未成功投递到交换机，返回nack
            +   channel.basicNack
    +   publisher-return，发生者回执
        +   消息投递到交换机了，但是没有路由到队列。返回ACK、路由失败原因。
        +   rabbitTemplate.setReturnCallback

2.  `MQ中丢失`：MQ收到消息，暂存内存中，还没消费，自己挂掉，数据会都丢失。这种情况**必须开启RabbitMQ的持久化**，就是消息写入之后会持久化到硬盘，哪怕RabbitMQ自己挂了，恢复之后也会自动读取之前存储的数据，一般数据不会丢失。设置持久化有两个步骤：第一个是创建queue的时候将其设置为持久化，这样可以保证RabbitMQ持久化queue的元数据，但是不会持久化queue里的数据。第二个是发送消息的时候，将消息的deliveryMode设置为2，就是将消息设置为持久化的，此时RabbitMQ就会将消息持久化到磁盘上去。必须同时设置这两个持久化才行，

3.  `消费端丢失`：消费的时候，刚消费到，还没处理，此时服务挂了，服务重启了，那么就尴尬了，RabbitMQ以为你消费了，这数据就丢了。这个时候需要使用RabbitMQ提供的ack机制，简单来说就是，关闭RabbitMQ的自动ack，然后在服务处理消息完毕后再**手动ack**，这样的话，如果消息还没处理完，那么就不会ack，消息就不会丢了。 

    **死信队列**、**消息补偿**机制
    
    ![image-20230716222416648](面试手册随笔.assets/image-20230716222416648.png)

消费出现异常：消息会不断requeue(重新入队）到队列，再重新发送给消费者，然后再次异常，再次requeue，无限循环，导致mq的消息处理飙升，带来不必要的压力。

我们可以利用Spring的retry机制，在消费者出现异常时利用本地重试，而不是无限制的requeue到mq队列。（yml中配置）
...
https://instaer.me/article/OLwzv2

https://blog.csdn.net/hc1285653662/article/details/129665173

https://blog.csdn.net/weixin_35794878/article/details/122952212

**注意：此条笔记只看解决方案就行**

**原因一**：

![在这里插入图片描述](面试手册随笔.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80NTQ5ODQ2NQ==,size_16,color_FFFFFF,t_70.png)

+   **解决方案一**：**拆分多个 Queue，每个 Queue一个 Consumer在应用端进行分片处理，对一组消息的统一标识进行Hash，散列到同一个queue**
    +   ![在这里插入图片描述](面试手册随笔.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80NTQ5ODQ2NQ==,size_16,color_FFFFFF,t_70-16937319777913.png)
    +   设置单个消费者会造成消息处理的并行度下降，可以在consumer 里面进行多线程消费提高速度

**原因二**：一个queue对应一个consumer去消费， **但是 consumer 里面进行了多线程消费， 无法保证哪个线程先执行完**，可能导致顺序错乱

+   **解决方案二**：**一个 Queue 对应一个 Consumer**，然后这个 Consumer 内部用多个内存队列做排队，然后分发给底层不同的 Worker（线程） 去处理内存队列，**相当于把外部的queue迁移到消费者内部了**

    ![image-20230903173704539](面试手册随笔.assets/image-20230903173704539.png)

### 大量消息在MQ里长时间积压，该如何解决？

当生产者发送消息的速度超过了消费者处理消息的速度，就会导致队列中的消息堆积，直到队列存储消息达到上限。最早接收到的消息，可能就会成为死信，会被丢弃，这就是**消息堆积**问题。

+ 增加更多消费者，提高消费速度
+ 在消费者内开启线程池加快消息处理速度
+ 扩大队列容积，提高堆积上限
	+ 即设置惰性队列
	+ ![image-20230919233519976](面试手册随笔.assets/image-20230919233519976.png)
...
<img src="面试手册随笔.assets/010b3ddbfb19418f8752c4070f038726.png" alt="img" style="zoom:67%;" />
...
当前执行器正在执行任务还没有结束时调度中心进行任务调度，此时该如何处理。
...
优点：用户只需要关心所需产品对应的工厂，无须关心创建细节；加入新产品**符合开闭原则**，提高可扩展性。
...
/*
为什么要用volatile：java里的new操作，底层其实经历了三步，1、为对象开辟内存空间，2、给对象初始化，3、将变量指向内存空间
第3步标志着对象创建的成功，若没有volatile修饰，第二步和第三步可能会发生重排序，第三步先于第二步执行，当指针指向内存空间后，对象的创建就已经结束，但是由于对象没有初始化，返回的是依然是null。
实际上volatile禁止的指令重排序不是2、3步的重排序，而是更加外层的对象的赋值和读取操作，这样即便底层2、3步发生了重排序，也不会产生任何影响
*/
/*
...
第二重空判断的作用：当多个线程⼀起到达锁位置时，进⾏锁竞争，其中⼀个线程获取锁，如果是第⼀次进⼊则为 null，会进⾏单例对
象的创建，完成后释放锁，其他线程获取锁后就会被空判断拦截，直接返回已创建的单例对象。
*/
//静态内部类
public class Singleton {
    private Singleton(){}
...
//枚举，Java虚拟机会保证枚举对象的唯一性，因此每一个枚举类型和定义的枚举变量在JVM中都是唯一的
//Java虚拟机会保证枚举类型不能被反射并且构造函数只被执行一次。
//enum类的本质其实是一个final修饰的类
...
都是**一堆命令批量执行、保证隔离性、具备一定的原子性**（**出错不会回滚**，这些错误应该在开发的过程中被发现，而不应该出现在生产环境）

lua是在事务的基础上增加了逻辑性，即if-else等
...
-   **复用**。客户端发送的脚本会永久存在 Redis 中，其他客户端可以复用这一脚本而不需要使用代码完成相同的逻辑。
    ...
    1.  为了避免 Redis 阻塞，Lua 脚本业务逻辑不能过于复杂和耗时；（结合项目中点赞持久化功能中，本来打算lua优化，但因为耗时长，故放弃）
...
自定义handler实现Ordered，重写`int getOrder();`来指定顺序，再写一个handler方法去执行相应的参数校验逻辑。

```java
//0：用户注册参数必填检验
@Component
@RequiredArgsConstructor
public final class UserRegisterParamNotNullChainHandler implements Ordered {
...
编写**责任链上下文类**，在service中直接调用handler方法
...
（1）性能问题。每个请求都是从链头开始遍历，特别是在链比较长的时候，性能较差。
...
+   缓存null值虽然可以，但是如果有大量并发**不同的非法请求**，每个请求都会先打到数据库。

+   使用布隆过滤器，将**所有已注册的用户名存入布隆过滤器**，判断时先判断该用户名是否在布隆过滤器中，不在的一定不存在，避免直接查询数据库。
...
+   Redis Set 存储已注册用户名
    ...
    +   此外，对于这么多的用户信息，是否应该将其存储在一个 Key 中呢？显然是不可行的。即使进行分片，也会增加系统的复杂度。
        结论：由于该方案**占用内存较多且复杂度较高**，因此不适合实际应用

最终采用**布隆过滤器+set**

set中只存储注销用户的用户名，大大减少了内存占用
...
+   如果一个**没有被创建过**的账号，在Redis Set 结构中不存在，但又被**误判存在于布隆过滤器**中的话怎么办

    +   可以把布隆过滤器适当调大，减少 Hash 冲突的可能性。然后，从业务角度上说，误判出现用户名不可用对用户来说也没什么，假如我申请用户名 AAA 不可用，我完全可以在原名称上再加一个A。

    

如果用户频繁申请账号再注销，可能导致用户注销可复用的 Username Redis Set 结构变得庞大，增加了存储和查询的负担。为了防止这种情况，我采取了以下解决方案：
...
![布隆过滤器介绍及原理、集成Redisson布隆过滤器解决Redis缓存穿透问题_Redis_03](面试手册随笔.assets/23134827_63f6fe2b319ad60168.png)
...
+   逻辑过期
    ...
    +   新建一个redisData实体，包含data和expireTime两个属性，存入Redis，expireTime为逻辑过期设置的时间
    ...
    +   ```java
        ...
        /过期,需要缓存重建
                ...
                if (tryLock(lockKey)) {
                    ...
                    //获取锁成功后再次检测redis缓存是否存在(逻辑过期),DoubleCheck
                    //因为成功获取互斥锁有可能是一个线程刚释放完互斥锁，也就证明此时缓存中的数据是新鲜热乎的，
                    //此时不需要再去重建缓存，刚刚有线程才重建完
                    // 存在则无需缓存重建
...
限流-->lua扣减Redis库存+用户已买-->创建订单-->MQ异步发消息-->Redission加锁保证一人一单-->乐观锁扣减MySQL库存-->订单落库-->（重试-->补偿服务）

##### 令牌桶限流

用的spring-cloud-starter-gateway的限流脚本request_rate_limiter.lua
...
public @interface Limiter {
    ...
    // 每次请求消耗令牌数量n
    // 如果桶中的令牌数小于n,则请求阻塞或直接放弃,否则顺利从桶中取走n个令牌
    int requestedTokens() default 1;
}

//切点
@Around("execution(public * *(..)) && @annotation(com.hmdp.limiter.Limiter)")
```

在非秒杀场景下也可使用验证码来强制限流，防止刷接口
...
+   判断库存是否充足，string结构
+   判断用户是否下过单，set结构，ismember命令
+   扣库存，seckill:stock:优惠券id:优惠券数量
+   set结构增加该用户的下单信息，seckill:order:优惠券id:用户id

##### 生成订单唯一ID

Redis 64位 string  时间戳：以秒为单位，可以用69年；序列号：秒内的计数器，支持最多每秒2^32个不同的id

![image-20230831221217490](面试手册随笔.assets/image-20230831221217490.png)

##### MQ异步消费

###### MQ配置

直连型交换机。声明队列、交换机、绑定队列和交换机

继承ApplicationContextAware，重写setApplicationContext，设置交换机到队列失败时的回调函数
...
Redis中库存扣减成功后发送消息给MQ，由MQ异步消费消息，进行MySQL的库存扣减

**少卖问题**：扣减库存后，发送消息失败，订单生成失败。此时要有消息发送失败的重试策略，超过重试次数后，就要将消息持久化，由补偿服务去扫描，进行后续处理。

rabbitTemplate.convertAndSend(exchange,routingKey,object,correlationData);

**携带有订单的全局唯一ID**，这个ID用于解决重复消费问题：发消息时候，在发送消息前，给消息设置一个唯一ID，消费者拿到消息时候，获取这个唯一ID然后保存到redis，然后消费。如果后续发过来同样的消息，就去redis查，如果有那就把这个消息放弃就行了
...
Redission分布式加锁确保此人多线程抢票时只有一个线程能去进行抢票操作；

数据库查询是否已存在订单（用户是否已购买）
...
+  超卖秒杀 就提 用令牌桶 验证码 redis预减 mq去乐观改数据库 这样的思路
...
为了保证消息不被重复消费，首先要保证每个消息是唯一的，所以可以给每一个消息携带一个**全局唯一的id，**流程如下**：**
...
校验法即通过校验码的数学计算的方式，对出现丢失、损坏的数据进行校验、还原。注意，这里有两个作用，一个校验，通过对数据**进行校验和进行计算，可以检查数据是否完整，有无损坏或更改**，在数据传输和保存时经常用到，如 TCP 协议；二是**恢复还原，通过对数据结合校验码，通过数学计算，还原丢失或损坏的数据**，可以在保证数据可靠的前提下，降低冗余，如单机硬盘存储中的 RAID 技术，纠删码技术等。MinIO 采用的就是纠删码技术。

### MinIO

<img src="面试手册随笔.assets/pS7VqFH.png" alt="img" style="zoom: 50%;" />

- MinIO使用**纠删码**技术来保护数据，它是一种恢复丢失和损坏数据的数学算法，它将数据分块，冗余地分散存储在各个节点的磁盘上，所有可用的磁盘组成一个集合，上图由8块硬盘组成一个集合，**当上传一个文件时，会通过纠删码算法计算对文件进行分块存储，除了将文件本身分成4个数据块，还会生成4个校验块，数据块和校验开会分散的存储在这8块硬盘上**
- 使用纠删码的好处是即便丢失一半数量(N/2)的硬盘，仍可以恢复数据。例如上面集合中有4个以内的硬盘损害，仍可保证数据恢复，不影响上传和下载；但如果多余一半的硬盘损坏，则无法恢复。
- 关于纠删码，简单来说就是可以通过数学计算，把丢失的数据进行还原，它可以将n份原始数据，增加m份数据，并能通过n+m份中的任意n份数据，还原为原始数据。即如果有任意小于等于m份的数据失效，仍然能通过剩下的数据还原出来。举个最简单例子就是有两个数据(d1, d2)，用一个校验和y（`d1 + d2 = y`）即可保证即使丢失其中一个，依然可以还原数据。如丢失 d1 ，则使用 `y - d2 = d1` 还原，同理，d2 丢失或者y丢失，均可通过计算得出。

### 断点续传

#### 流程

1. 前端上传文件前，根据md5值去数据库查看是否存在
	- 若存在，则不再上传
	- 若不存在，则开始上传，首先对视频文件进行分块
2. 前端分块进行上传，上传前首先检查分块是否已经存在，去MinIO中，根据一定的规则去指定文件目录下查找（文件名：abcd，存储路径：/a/b/abcd/chunk/0  1  2  3）
	- 若分块已存在，则不再上传
	- 若分块不存在，则开始上传分块
3. 前端请求媒资管理接口层，请求上传分块
4. 服务端将分块信息上传到MinIO
5. 前端将分块上传完毕，请求接口层合并分块
6. 服务层根据文件信息找到MinIO中的分块文件，下载到本地临时目录，将所有分块下载完毕后开始合并
7. 合并完成**校验合并后的文件是否完整**，如果完整则**上传完成并删除分块**，否则删除文件。
...
https://zhuanlan.zhihu.com/p/625060354

![img](面试手册随笔.assets/v2-d4cc908439e53c1606bd751d9dd8eff9_720w.webp)

https://blog.csdn.net/qq_40991313/article/details/129766117

#### 为什么要进行视频转码（avi->mp4）

+   AVI 文件往往比其他视频文件格式大，因为它们使用较少的压缩，从而可以产生更高的质量。不幸的是，这也会使 AVI 文件变得非常大 — 由于文件很大，因此存储或共享它们具有挑战性。

	MP4 文件使用有损压缩算法，可在减小文件大小的同时保留高质量的视频和音频。此过程的工作原理是丢弃视频中一些不太重要的信息，从而在不影响重要细节或质量的情况下减小其大小。

+   MP4 文件体积更小，兼容各种平台

+   如果正在创建专业的内容并且需要高质量的内容，AVI 可能更可取，因为它提供无损压缩以保持原始视频的质量。

#### xxljob在分片广播的策略下，怎么保证执行器之间查询不到重复的任务？

（注意此处是**查询不到重复**的，不是**不重复执行**）

+ 采用作业分片的方案：
	+ 每个执行器收到广播任务有两个参数：分片总数、分片序号。每个执行从数据表取任务时可以让**任务id % 分片总数**，如果等于分片序号则执行此任务。
	+ <img src="面试手册随笔.assets/image-20230923190856684.png" alt="image-20230923190856684" style="zoom: 80%;" />

#### xxljob如何保证任务不重复执行？

{如果同一个执行器，在处理一个视频的时候，还没有处理完（因为网络问题没有处理完，导致调度超时），此时调度中心又来了一次请求调度，为了不重复处理同一个视频，该怎么办？

+ **调度过期策略为“忽略” ，阻塞处理策略为“丢弃后续调度”。**
...
+   分布式锁（乐观锁的形式）再次保证任务不重复执行：理论上每个执行器分到的任务是不重复的，但是当在执行器弹性扩容时无法绝对避免任务不重复执行，比如：原来有四个执行器正在执行任务，由于网络问题原有的0、1号执行器无法与调度中心通信（此时0/1已经在执行任务了），调度中心就会对执行器重新编号（因为弹性扩容机制，原来的3、4此时就变成了0、1），原来的3、4执行器可能就会执行和0、1号执行器相同的任务。
	...
	这里只需要依靠任务的状态即可实现（未处理`1`；处理中`2`；处理失败`3`；处理成功`4`），可以看到这里类似于 CAS 的方式**通过比较和设置的方式只有在状态为未处理或处理失败时才能设置为处理中**。这样在并发场景下，即使多个执行器同时处理该任务，也只有一个任务可以设置成功进入处理任务阶段。

	为了真正达到幂等性，还需要设置一下 xxl-job 的**调度过期策略和阻塞处理策略**来保证真正的幂等性。分别设置为 **忽略**（调度过期后，忽略过期的任务，从当前时间开始重新计算下次触发时间） 和 **丢弃后续调度**（调度请求进入单机执行器后，发现执行器存在运行的调度任务，本次请求将会被丢弃并标记为失败，**丢弃也没事下一次调度就又可以执行了**）

#### 流程

通过 **分片广播拿到的参数以取模的方式** 获取当前执行器所属的任务记录集合

· 遍历集合，以 **多线程的方式** 并发地执行任务

· 每次执行任务前需要先通过 **数据库乐观锁的方式** 抢占当前任务，抢占到才能执行

· 执行任务过程分为 分布式文件系统下载需要转码的视频文件 -> 视频转码 -> 上传转码后的视频 -> 更新任务状态（处理成功）

· 使用JUC工具类 CountDownLatch 实现所有任务执行完后才退出方法

· 中间使用 xxl-job 的日志记录错误信息和执行结果



1. **清理任务表中转码成功的任务的记录并将其插入任务历史表**

由于任务表处理完任务后只是更新任务状态，这样随着任务增多会导致检索起来时间消耗过大，所以使用任务调度的方式定期扫描任务表，将任务状态为处理成功的任务删除并重新插入任务历史表中留存（由于代码过于简单，这里就不做展示了）。

主要实现两个功能：① 清理任务表中已成功处理的任务； ② 将处理成功的任务记录插入历史表中；

​	2.**视频补偿机制**

由于使用乐观锁会将任务状态更新为处理中，如果此时执行任务的执行器（服务）宕机了，会导致该任务记录一直存在，因为乐观锁的原因别的执行器也无法获取，这个时候同样需要使用任务调度的方式，定期扫描任务表，**判断任务是否处于处理中状态并且任务创建时间远大于30分钟，则说明任务超时了**，则是使用任务调度的方式重新更新任务的状态为未处理，等待下一次视频转码任务的调度处理。此外视频补偿机制任务调度还需要**检查是否存在任务最大次数已经大于3次的**，如果存在则交付给人工处理（由于代码过于简单，这里就不做展示了）。
...
请求先进入到漏桶里，漏桶以固定的速度出水，也就是处理请求，当水加的过快，则会直接溢出，也就是拒绝请求，可以看出漏桶算法**能强行限制数据的传输速率**
...
系统会以一个恒定的速度往桶里放入令牌，而如果请求需要被处理，则需要先从桶里获取一个令牌，当桶里没有令牌可取时，则拒绝服务。**允许一定程度上的突发传输**

项目中使用springcloud-gateway中的限流lua脚本自定，结合自定义注解+AOP的@ Around（可以仅用@Before），实现了更细粒度的，某个接口的速率控制

可以根据userId或IP进行限流，具体区别是redis存放的key不一样
